...
export default class OrphanageValidator {
  static validate({
    ...
    const openOnWeekendsFieldErrors = this.validateOpenOnWeekends(
      openOnWeekends,
    );
  ...
  static validateOpeningHours(openingHours: string) {
    ...
    if (openingHours.length > maxLength) {
      errors.push(
        'O campo "horário de abertura" deve ter no máximo 80 carácteres.',
      );
    }
  ...
  static validateOpenOnWeekends(openOnWeekends: boolean) {
    ...
    if (isEmpty) {
      errors.push(
        'Você deve marcar se o orfanato abre ou não aos finais de semana.',
      );
    }
